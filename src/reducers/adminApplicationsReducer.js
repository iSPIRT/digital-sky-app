...
const initialState = { loadingApplications: false, approvingApplication:false, errors:[] };
...
export function adminApplications(state = initialState, action) {
    switch (action.type) {
        ...
        case LOAD_APPLICATIONS_SUCCESS:
          return { ...state, loadingApplications:false, [action.applicationType]: action.applications };
        ...
        case APPROVE_APPLICATION_SUCCESS:
          ...
          const indexOfUpdatedApplication =  applications.findIndex( application => (application.id === updatedApplication.id))
          const updatedApplications=  [
                  ...applications.slice(0, indexOfUpdatedApplication),
                  updatedApplication,
                  ...applications.slice(indexOfUpdatedApplication+1)
          ]
          return { ...state, approvingApplication:false, [action.applicationType]: updatedApplications};
