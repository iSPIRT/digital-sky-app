...
export function operatorProfile(state = initialState, action) {
  switch (action.type) {
    ...
    case SAVE_OPERATOR_PROFILE_REQUEST:
      return {
        ...state,
        savingOperatorProfile: true,
        operatorProfileSaved: false,
        errors: []
      };
    case SAVE_OPERATOR_PROFILE_SUCCESS:
      return {
        savingOperatorProfile: false,
        operatorProfileSaved: true,
        errors: [],
        profile: action.operatorProfile
      };
