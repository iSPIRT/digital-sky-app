...
export const SEND_RESET_PASSWORD_LINK_REQUEST =
...
export const sendResetPasswordLink = email => {
  return dispatch => {
    dispatch(request());
    userService.sendResetPasswordLink(email).then(
      data => {
        dispatch(success());
      },
      errors => {
  ...
  function request() {
    return { type: SEND_RESET_PASSWORD_LINK_REQUEST };
  }
  function success() {
    return { type: SEND_RESET_PASSWORD_LINK_SUCCESS };
  }
  function failure(errors) {
    return { type: SEND_RESET_PASSWORD_LINK_FAILURE, errors };
  }
};
...
export const resetPassword = payload => {
  ...
  function failure(errors) {
    return { type: RESET_PASSWORD_FAILURE, errors };
  }
};
