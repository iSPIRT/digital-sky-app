...
function loadAirspaceCategories() {
    ...
    return fetch(apiRoot + "/airspaceCategory/list", requestOptions).then(handleResponse).then(mergeAirspaceCategoriesByType);
}
    ...
    airspaceCategories.forEach(airspaceCategory => {
        if(airspaceCategory.type === 'GREEN'){
            if(result.greenCategories){
                result.greenCategories.features = [...result.greenCategories.features, ...airspaceCategory.geoJson.features]
            } else   {
        ...
        if(airspaceCategory.type === 'AMBER'){
            if(result.amberCategories){
                result.amberCategories.features = [...result.amberCategories.features, ...airspaceCategory.geoJson.features]
            } else   {
        ...
        if(airspaceCategory.type === 'RED'){
            if(result.redCategories){
                result.redCategories.features = [...result.redCategories.features, ...airspaceCategory.geoJson.features]
            } else   {
